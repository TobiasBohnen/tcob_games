...
private:
    template <typename T>
    auto create(data::config::object const& obj, style_collection& styles, std::vector<std::string> const& names, flags flags);
...
template <typename T>
inline auto styles::create(data::config::object const& obj, style_collection& styles, std::vector<std::string> const& names, flags flags)
{
