...
void main() {
    vec2 uv = curveUV(vs_tex_coords.xy);
    if (uv.x < 0.0 || uv.x > 1.0 || uv.y > 0.0 || uv.y < -1.0) {
        discard;
    }
    
    ivec2 tex_size      = textureSize(texture0, 0).xy;
    ivec2 pixel_coords  = ivec2(uv.x * float(tex_size.x), abs(uv.y) * float(tex_size.y));
    ivec2 time_seed     = ivec2(int(floor(global.time / 80.0))); 
    ivec2 block_id      = ivec2(float(pixel_coords.x) / (12.0 + 12.0 * hash(time_seed)), float(pixel_coords.y));
    ...
    float r   = texture(texture0, vec3(uv.x + shift, uv.y, vs_tex_coords.z)).r;
    float g   = texture(texture0, vec3(uv.x,         uv.y, vs_tex_coords.z)).g;
    float b   = texture(texture0, vec3(uv.x - shift, uv.y, vs_tex_coords.z)).b;
    vec3 rgb  = vec3(r, g, b);
    
    vec3 ghost      = texture(texture0, vec3(uv.x - 0.008, uv.y, vs_tex_coords.z)).rgb * 0.1;
    rgb += ghost;
    
    float dotcrawl  = sin((float(pixel_coords.x) + float(pixel_coords.y) + global.time * 2.0) * 3.14159) * 0.05 * length(rgb);
    rgb += dotcrawl;
    ...
    fragColor = vec4(rgb * scanline * mask * flicker * vignette, 1.0);
}