    ...
    , _state {state}
{
    ...
    btn0.Label = "GO";
    btn0.Click.connect([&events]() { emit_signal(events.TurnStart); });
    ...
    btn1.Label = "RESET";
    btn1.Class = "button2";
    btn1.Click.connect([&events]() { emit_signal(events.GameRestart); });

    auto& btn2 {layout2.create_widget<button>({3, 2, 1, 2}, "btn2")};
    btn2.Label = "OFF";
    btn2.Class = "button2";
    btn2.Click.connect([&events]() { emit_signal(events.GameQuit); });
