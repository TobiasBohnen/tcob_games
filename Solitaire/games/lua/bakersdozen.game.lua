...
local capricieuse                      = {
    ...
    redeal         = function(game) return game:reshuffle_tableau() end,
    before_shuffle = function(game, card)
        if card.Rank == "Ace" then
            return game.put_card(card, game.Foundation, 0, 4)
        end
        if card.Rank == "King" then
            return game.put_card(card, game.Foundation, 4, 4)
        end
...
cruel.redeal                           = function(game) return game:redeal_tableau() end
...
indefatigable.Tableau.create           = function()
    return {
        Initial = piles.initial.face_up(4),
        Layout = "Column",
        Rule = { Build = "UpOrDownInSuit", Move = "Top", Empty = "Any" }
...
perseverance.Tableau.create            = function()
    return {
        Initial = piles.initial.face_up(4),
        Layout = "Column",
...
royal_family.Foundation.create         = function() return { Rule = { Build = "DownInSuit", Move = "Top", Empty = "King" } } end
royal_family.Tableau.create            = function()
    return {
        Initial = piles.initial.face_up(4),
        Layout = "Column",
...
portuguese_solitaire.Tableau.create    = function()
    return {
        Initial = piles.initial.face_up(4),
        Layout = "Column",
...
spanish_patience.Foundation.create     = function() return { Rule = { Build = "UpAlternateColors", Move = "Top", Empty = "Ace" } } end
spanish_patience.Tableau.create        = function(i)
    return {
        Initial = piles.initial.face_up(4),
        Layout = "Column",
        Rule = { Build = "DownByRank", Move = "Top", Empty = "None" }
...
spanish_patience_2.Tableau.create      = function()
    return {
        Initial = piles.initial.face_up(4),
        Layout = "Column",
...
register_game(bakers_dozen)
register_game(bakers_dozen_two_decks)
register_game(capricieuse)
register_game(castles_in_spain)
register_game(cruel)
register_game(fifteen)
register_game(good_measure)
register_game(indefatigable)
register_game(martha)
register_game(nationale)
register_game(perseverance)
register_game(portuguese_solitaire)
register_game(ripple_fan)
register_game(royal_family)
register_game(spanish_patience)
register_game(spanish_patience_2)
register_game(strata)
register_game(unusual)
register_game(vineyard)
